...
### 推荐阅读顺序（重组视图）
1) 数据库函数提效（强烈推荐先读）
2) 0. 总览与关键约定（单位、时间粒度、可视化契约）
3) A. 判定规则与方案选择（A1–A5，一页清晰版）
4) 1–3. 基础算法与策略（FFILL/MEAN/REG 与决策树/评分）
5) 四–七. 工程化实现（数据补齐/计算/优化流程与表结构）
6) 八–十. 可视化/API/性能与监控
7) 十一与附. 风险回退、与仓库对齐点、代码示例、任务拆解
...
### 在本文中的落位（已加提示/对齐）
- 3）与现有网关/报表函数的结合：新增“优先使用 public.get_*_by_time_range，跨度大再用 reporting.get_metrics_auto_multi”。
- 七、只读 API：说明已有端点内部使用上述函数，统一返回包裹不变。
- 10. 秒级处理：补充“前端秒级回放优先用 get_*_by_time_range，避免 API 层二次拼装”。
...
- 决策树（统一）：
  ...
  2) 否则 若 L ≤ mean_limit_sec 且邻域覆盖率≥coverage_min → MEAN
  3) 否则 若 REG 模型可用且质量达标（R²≥r2_min 且样本≥n_min）→ REG
  4) 否则 放弃补齐（写入告警与 completion_audit）
- 指标类别默认策略（可被策略表覆盖）：
...
- coverage_min：0.5；jump_thr 与 bounds_* 按 metric_key 业务值配置
...
- 关键变量（metric_key）：main_pipeline_*、pump_*、water_temperature 等；优先使用已配置在 dim_metric_config 的 metric_key。
...
1）缺失检测（按设备/指标/秒级桶）
- 依据 dim_metric_config.metric_key 选取关键指标（如 main_pipeline_*、pump_*）。
...
1）步骤（每分钟批）
- ingest 原始数据（输入：main_pipeline_*、pump_*、water_temperature）
...
## 七、只读 API（供可视化）
1）计算过程（数据源说明：底层已采用 get_*_by_time_range 或 reporting.get_metrics_auto_multi，按窗口长度自适应；统一返回包裹不变）
...
- 第 3–4 天：
  ...
  - 可视化 API（compute 系列）
- 第 5–6 天：
  - 优化流程门控/残差/参数递推（RLS + 限幅），写 optimize_* 与 calibration_log
  - 可视化 API（opt 系列）
- 第 7–8 天：
...
- 写入策略：
  - 缺失补齐：INSERT ... ON CONFLICT (station_id,device_id,metric_id,ts_bucket)
    DO UPDATE SET value=EXCLUDED.value, source_hint='imputed_*', ts_raw=now()
  - 异常替换：建议先写 completion_audit，再 UPDATE fact_measurements 设置 value=新值、source_hint='imputed_*_override'
- 版本控制：
...
## 6. 可视化支撑表（补齐专用）
为避免与 compute_* 混淆，单独引入 completion_* 表（也可共用 compute_*，此处推荐拆分）。
```
...
## 任务拆解（更新版，含模块归属）
- D1–D2 数据库：创建 completion_* / compute_* / optimize_* / calibration / recommendations 表与索引
