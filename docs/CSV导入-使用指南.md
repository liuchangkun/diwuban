...
## 快速开始
1) 准备映射与 CSV（标准 schema：config/data_mapping.v2.json）
2) 检查映射（只读）
   - `python -m app.cli.main check-mapping config/data_mapping.v2.json`
3) 维表与建表
   - `python -m app.cli.main prepare-dim config/data_mapping.v2.json`
   - `python -m app.cli.main create-staging`
4) 导入与合并（auto 模式按阈值自动选择 direct/staging）
   - direct（小量）示例：`python -m app.cli.main ingest-direct config/data_mapping.v2.json`
   - staging（大吞吐）示例：`python -m app.cli.main ingest-copy config/data_mapping.v2.json`
   - 合并：`python -m app.cli.main merge-fact --window-start 2025-08-11T00:00:00Z --window-end 2025-08-18T00:00:00Z`
5) 一键（auto 决策）
   - `python -m app.cli.main run-all config/data_mapping.v2.json --window-start ...Z --window-end ...Z`
...
- check-mapping（只读一致性检查）
  ...
  - 输出：JSON 报告（base_dir、总路径数、带 data/ 前缀数、存在数、逐条建议、group_by_* 聚类统计）
...
- 维表与映射：
  - `python -m app.cli.main prepare-dim config/data_mapping.v2.json --log-run --log-dir logs/runs/pd01`
- 创建 staging：
  - `python -m app.cli.main create-staging --log-run --log-dir logs/runs/st01`
- COPY 导入：
  - `python -m app.cli.main ingest-copy config/data_mapping.v2.json --log-run --log-dir logs/runs/ic01`
- 合并：
...
- 位置：configs/ingest.yaml（ENV 覆盖支持：INGEST_*）
