...
## 1A. 面向对象设计六大原则（SOLID + LoD）

本节补充落实“设计原则与边界”，将六大设计原则作为评审/实现的强约束与落地清单，指导 app/ 的模块化与演进。

- 目标：降低耦合、提升可维护性与可扩展性，避免回归风险与过度修改
- 适用：模块/类/函数/接口设计与重构、评审 Checklist、自适应扩展点的抽象

### 1A.1 单一职责原则（SRP）
- 定义：一个模块/类/函数仅有一个引起其变化的原因（职责单一、聚焦领域）
- 要点：
  - 文件/类过大、函数>200 行、混合 IO/业务/汇总逻辑 → 必拆分（见 2、31A）
  - 入口层不写业务；跨层交互经网关（adapters）
  - 公共 API 以“动词+对象”表达单一动作
- 本项目落地：
  - 目录分层 app/{adapters, services, core, cli}；merge/copy 分职（见 35）
  - Docstring 明确职责/依赖/示例；公开 API 全量类型标注（见 4、32）

### 1A.2 开闭原则（OCP）
- 定义：对扩展开放，对修改封闭；新增能力优先“加实现/配置”，而非“改老代码”
- 要点：
  - 以抽象（Protocol/接口）+ 策略（Strategy）承载变化点
  - 避免 if-else 硬编码分支，改为“注册表/工厂/策略映射”
  - 用配置驱动可变参数与阈值（见 12、20）
- 本项目落地：
...
- 要点：
  - 不加强前置条件/不放宽后置条件；返回值/异常契约一致
  - 输入输出类型与语义保持父类/协议契约（见 32 的 Protocol）
- 本项目落地：
  - 适配器实现（DbGateway/SqlGateway）可替换；测试以协议为准，不依赖具体类
...
- 本项目落地：
  - 将 db/logging/fs 接口按操作场景拆分；服务层仅依赖所需协议方法

### 1A.5 依赖倒置原则（DIP）
- 定义：高层模块不依赖低层实现，二者都依赖抽象；细节依赖抽象
- 要点：
  - 通过构造注入/工厂注入依赖；禁止在业务内直接 new 具体实现
  - 配置集中在 settings，依赖由入口组装注入（见 12）
- 本项目落地：
  - services 依赖 adapters 的协议（Protocol），实例由 cli/main 组装注入

### 1A.6 迪米特法则（LoD，最少知识原则）
- 定义：只与“直接朋友”通信，避免深层对象链条（a.b.c.d）
- 要点：
  - 提供合适的领域方法，避免暴露内部结构；减少链式穿透
  - 限制跨层访问，统一通过网关/服务接口交互
- 本项目落地：
  - 严禁在业务层直接操作底层连接/游标；通过 DbGateway 统一访问（见 7、31A）

### 1A.7 评审自检清单（落地）
- [ ] 职责单一：模块/类/函数是否仅一个变化原因？是否存在混杂 IO/业务/汇总？
- [ ] 扩展优先：新增需求是否通过“新增实现/配置”而非“改老逻辑”？
- [ ] 可替换：实现是否完全满足协议契约（类型/异常/语义）？
- [ ] 小接口：是否为调用方裁剪了最小必要接口？
- [ ] 依赖注入：高层是否仅依赖抽象？实例由入口装配？
- [ ] 最少知识：是否消除了 a.b.c 级联访问与跨层直连？
