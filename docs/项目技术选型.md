...
## 应用层（FastAPI + Pydantic v2 + psycopg3）

- 选择理由：
  - FastAPI：类型友好、异步支持、路由清晰
  - Pydantic v2：更快的数据校验与模型定义
  - psycopg3：现代 PG 客户端，支持同步高性能连接池
- 替代与取舍：
  - Flask：轻量但类型/异步生态弱
  - Django：重量级，ORM 与我们"以函数为接口"的策略不完全吻合
  - SQLAlchemy ORM：对象映射便利，但我们强调 DB 函数为接口，采用轻量 SQL 层
  - asyncpg：异步性能优秀，但项目当前采用同步模式，统一使用 psycopg3
- 版本与升级：
  - FastAPI >=0.110，Pydantic v2，psycopg >=3.1
  - 升级遵循"接口稳定 + 集成测试先行"
...
- 应用：FastAPI + Pydantic v2 + psycopg3
