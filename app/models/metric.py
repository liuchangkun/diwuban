...
class MetricInfo(BaseModel):
    ...
    unit: Optional[str] = Field(
        default=None, description="单位（优先 unit_display 回退 unit）"
    )
    unit_display: Optional[str] = Field(
        default=None, description="单位展示（可能为空）"
    )
    value_type: Optional[str] = Field(
        default=None, description="值类型，如 numeric/text"
    )
    fixed_decimals: Optional[int] = Field(
        default=None, description="固定小数位（可选）"
    )
