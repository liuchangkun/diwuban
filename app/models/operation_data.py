...
说明（历史兼容）：
- 本模块中的 OperationData 模型对应历史视图/表 operation_data，但当前数据库中已不再提供该视图；
- 新代码应优先使用 fact_measurements + 维表，或数据库函数
  public.get_device_metrics_by_time_range / public.get_station_devices_metrics_by_time_range
  与报表函数 reporting.get_metrics_* 进行查询与聚合；
- 本文件保留作为数据结构参考与对照，不建议在新代码中直接依赖 operation_data。

本模块定义了运行时序数据相关的模型，包括：

核心模型：
- OperationData：运行数据完整模型（历史兼容模型）
...
class OperationData(BaseModel, ValidationMixin):
    ...
    id: Optional[int] = Field(description="数据记录ID", default=None)

    station_id: StationId = Field(description="泵站ID", max_length=50)

    device_id: DeviceId = Field(description="设备ID", max_length=50)

    timestamp: datetime = Field(description="数据时间戳")

    # 主要运行参数
    flow_rate: Optional[Decimal] = Field(
        description="流量（m³/h）", default=None, ge=0, decimal_places=3
    )

    pressure: Optional[Decimal] = Field(
        description="压力（MPa）", default=None, ge=0, decimal_places=3
    )

    power: Optional[Decimal] = Field(
        description="功率（kW）", default=None, ge=0, decimal_places=3
    )

    frequency: Optional[Decimal] = Field(
        description="频率（Hz）", default=None, ge=0, le=100, decimal_places=2
    )

    # 状态和质量信息
    status: Optional[int] = Field(description="运行状态代码", default=None, ge=0)

    data_source: Optional[str] = Field(
        description="数据源标识", default=None, max_length=20
    )
...
class MeasurementData(BaseModel, ValidationMixin):
    ...
    id: Optional[int] = Field(description="行ID（非主键）", default=None)

    station_id: int = Field(description="泵站维度ID", gt=0)

    device_id: int = Field(description="设备维度ID", gt=0)

    metric_id: int = Field(description="指标维度ID", gt=0)

    ts_raw: datetime = Field(description="原始时间戳")

    ts_bucket: datetime = Field(description="对齐时间戳（秒级）")

    value: Decimal = Field(description="测量数值", decimal_places=6)

    source_hint: Optional[str] = Field(description="来源标识", default=None)

    inserted_at: Optional[datetime] = Field(description="插入时间", default=None)
...
class PumpOperationPoint(BaseModel):
    ...
    frequency: Optional[Decimal] = Field(
        description="频率（Hz）", default=None, ge=0, le=100
    )

    # 计算参数
    efficiency: Optional[Decimal] = Field(
        description="效率（%）", default=None, ge=0, le=100
    )

    specific_energy: Optional[Decimal] = Field(
        description="比能耗（kWh/m³）", default=None, ge=0
    )

    # 状态信息
    is_valid: bool = Field(description="数据有效性", default=True)
    quality_score: Optional[Decimal] = Field(
        description="质量评分（0-100）", default=None, ge=0, le=100
    )
...
class TimeSeriesQuery(BaseModel, ValidationMixin):
    ...
    station_id: Optional[StationId] = Field(description="泵站ID", default=None)

    device_id: Optional[DeviceId] = Field(description="设备ID", default=None)

    device_ids: Optional[List[DeviceId]] = Field(description="设备ID列表", default=None)

    metric_keys: Optional[List[MetricKey]] = Field(
        description="指标键列表", default=None
    )
    ...
    limit: Optional[int] = Field(
        description="结果数量限制", default=None, gt=0, le=10000
    )

    offset: Optional[int] = Field(description="结果偏移量", default=None, ge=0)

    aggregate_interval: Optional[str] = Field(
        description="聚合时间间隔（如 '1h', '5m'）", default=None
    )

    aggregate_functions: Optional[List[str]] = Field(
        description="聚合函数列表（如 ['avg', 'max', 'min']）", default=None
    )

    include_quality: bool = Field(description="是否包含数据质量信息", default=False)
...
class TimeSeriesResponse(BaseModel):
    ...
    total_count: Optional[int] = Field(description="总记录数", default=None)

    query_time_ms: Optional[float] = Field(description="查询耗时（毫秒）", default=None)

    has_more: bool = Field(description="是否有更多数据", default=False)
...
class DataExportRequest(BaseModel, ValidationMixin):
    ...
    format: str = Field(description="导出格式", default="csv")

    include_headers: bool = Field(description="是否包含表头", default=True)

    timezone: Optional[str] = Field(description="时区（如系统默认时区）", default=None)
