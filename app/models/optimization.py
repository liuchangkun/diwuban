...
class OptimizationAlgorithm:
    ...
    def all(cls) -> list[str]:
        """获取所有优化算法"""
        return [
            cls.GENETIC_ALGORITHM,
            cls.PARTICLE_SWARM,
            cls.DIFFERENTIAL_EVOLUTION,
            cls.SIMULATED_ANNEALING,
            cls.LINEAR_PROGRAMMING,
            cls.NONLINEAR_PROGRAMMING,
            cls.HEURISTIC,
        ]
...
class Optimization(BaseModel, TimestampMixin, ValidationMixin):
    ...
    opt_id: Optional[int] = Field(description="优化ID（自增主键）", default=None)

    station_id: StationId = Field(description="泵站ID", max_length=50)

    run_timestamp: datetime = Field(description="优化计算执行时间")
    ...
    expected_savings: Optional[Decimal] = Field(
        description="预期节能率（%）", default=None, ge=0, le=100, decimal_places=2
    )

    status: str = Field(description="优化状态", default=OptimizationStatus.PENDING)

    executed_by: Optional[str] = Field(
        description="执行者", default=None, max_length=50
    )

    executed_at: Optional[datetime] = Field(description="执行时间", default=None)

    # 扩展字段
    algorithm: Optional[str] = Field(description="使用的优化算法", default=None)

    algorithm_version: Optional[str] = Field(
        description="算法版本", default=None, max_length=20
    )

    constraints: Optional[Dict[str, Any]] = Field(
        description="约束条件（JSON格式）", default=None
    )

    objective_values: Optional[Dict[str, Decimal]] = Field(
        description="目标函数值", default=None
    )

    computation_time_ms: Optional[float] = Field(
        description="计算耗时（毫秒）", default=None, ge=0
    )

    confidence_score: Optional[Decimal] = Field(
        description="置信度评分（0-100）", default=None, ge=0, le=100
    )

    baseline_config: Optional[Dict[str, Any]] = Field(
        description="基准配置（用于对比）", default=None
    )
...
class OptimizationCreate(BaseModel, ValidationMixin):
    ...
    recommended_config: Dict[str, Any] = Field(description="推荐配置")
    ...
    algorithm: Optional[str] = Field(description="优化算法", default=None)

    constraints: Optional[Dict[str, Any]] = Field(description="约束条件", default=None)
    ...
    computation_time_ms: Optional[float] = Field(
        description="计算耗时", default=None, ge=0
    )
...
class OptimizationRequest(BaseModel, ValidationMixin):
    ...
    # 时间范围
    start_time: Optional[datetime] = Field(description="历史数据开始时间", default=None)

    end_time: Optional[datetime] = Field(description="历史数据结束时间", default=None)

    # 约束条件
    constraints: Dict[str, Any] = Field(description="约束条件", default_factory=dict)

    # 算法选项
    algorithm: str = Field(
        description="优化算法", default=OptimizationAlgorithm.GENETIC_ALGORITHM
    )

    max_iterations: Optional[int] = Field(
        description="最大迭代次数", default=1000, gt=0, le=10000
    )

    convergence_tolerance: Optional[Decimal] = Field(
        description="收敛容差", default=Decimal("0.001"), gt=0, le=1
    )

    # 目标权重（多目标优化时使用）
    objective_weights: Optional[Dict[str, Decimal]] = Field(
        description="目标权重", default=None
    )

    # 设备选择
    device_ids: Optional[List[DeviceId]] = Field(
        description="参与优化的设备ID列表", default=None
    )

    include_pump_curves: bool = Field(description="是否使用泵特性曲线", default=True)

    # 场景设置
    scenario_name: Optional[str] = Field(
        description="优化场景名称", default=None, max_length=100
    )

    base_load_profile: Optional[Dict[str, Any]] = Field(
        description="基础负荷曲线", default=None
    )
...
class OptimizationResult(BaseModel):
    ...
    optimization: Optional[Optimization] = Field(
        description="优化结果记录", default=None
    )

    # 性能指标
    improvement_percentage: Optional[Decimal] = Field(
        description="改进百分比", default=None
    )

    annual_savings_estimate: Optional[Decimal] = Field(
        description="年度节约估算（元）", default=None
    )

    payback_period_months: Optional[int] = Field(
        description="投资回收期（月）", default=None
    )

    # 详细分析
    current_performance: Optional[Dict[str, Any]] = Field(
        description="当前性能指标", default=None
    )

    optimized_performance: Optional[Dict[str, Any]] = Field(
        description="优化后性能指标", default=None
    )

    sensitivity_analysis: Optional[Dict[str, Any]] = Field(
        description="敏感性分析结果", default=None
    )

    # 实施建议
    implementation_steps: List[str] = Field(
        description="实施步骤", default_factory=list
    )

    risks: List[str] = Field(description="风险提示", default_factory=list)

    monitoring_suggestions: List[str] = Field(
        description="监控建议", default_factory=list
    )
...
class OptimizationResponse(Optimization):
    ...
    # 实施状态
    implementation_status: Optional[str] = Field(description="实施状态", default=None)

    actual_savings: Optional[Decimal] = Field(description="实际节约效果", default=None)

    feedback_score: Optional[int] = Field(
        description="用户反馈评分（1-5）", default=None, ge=1, le=5
    )
...
class OptimizationComparison(BaseModel):
    ...
    recommendation: Optional[str] = Field(description="推荐方案", default=None)

    recommendation_reason: Optional[str] = Field(description="推荐理由", default=None)
...
class OptimizationSchedule(BaseModel, ValidationMixin):
    ...
    schedule_id: Optional[int] = Field(description="调度ID", default=None)
    ...
    frequency: Optional[str] = Field(
        description="执行频率（如 'daily', 'weekly', 'monthly'）", default=None
    )

    next_run: Optional[datetime] = Field(description="下次执行时间", default=None)

    is_active: bool = Field(description="是否激活", default=True)

    auto_apply: bool = Field(description="是否自动应用结果", default=False)

    notification_emails: List[str] = Field(
        description="通知邮箱列表", default_factory=list
    )
