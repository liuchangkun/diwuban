...
class Device(BaseModel, TimestampMixin, ValidationMixin):
    ...
    device_id: DeviceId = Field(
        description="设备唯一标识符", max_length=50, pattern=r"^[A-Za-z0-9_-]+$"
    )

    station_id: StationId = Field(description="所属泵站ID", max_length=50)

    name: str = Field(description="设备名称", max_length=100, min_length=1)

    type: str = Field(description="设备类型", max_length=30)

    pump_type: Optional[str] = Field(
        description="泵类型（仅当 type='pump' 时有效）", default=None, max_length=30
    )

    model: Optional[str] = Field(description="设备型号", default=None, max_length=50)

    rated_power: Optional[Decimal] = Field(
        description="额定功率（kW）", default=None, ge=0, decimal_places=2
    )

    rated_flow: Optional[Decimal] = Field(
        description="额定流量（m³/h）", default=None, ge=0, decimal_places=2
    )

    rated_head: Optional[Decimal] = Field(
        description="额定扬程（m）", default=None, ge=0, decimal_places=2
    )

    manufacturer: Optional[str] = Field(
        description="制造商", default=None, max_length=100
    )

    install_date: Optional[date] = Field(description="安装日期", default=None)

    status: str = Field(description="设备状态", default=DeviceStatus.ACTIVE)
...
class DeviceCreate(BaseModel, ValidationMixin):
    ...
    pump_type: Optional[str] = Field(
        description="泵类型（仅当 type='pump' 时有效）", default=None
    )
...
class DeviceUpdate(BaseModel, ValidationMixin):
    ...
    name: Optional[str] = Field(
        description="设备名称", default=None, max_length=100, min_length=1
    )

    type: Optional[str] = Field(description="设备类型", default=None)
    ...
    status: Optional[str] = Field(description="设备状态", default=None)
...
class DeviceResponse(Device):
    ...
    # 可以添加计算字段或关联数据
    station_name: Optional[str] = Field(description="所属泵站名称", default=None)

    last_data_time: Optional[datetime] = Field(description="最后数据时间", default=None)

    data_count_24h: Optional[int] = Field(description="24小时内数据点数", default=None)
...
class PumpPerformanceSpec(BaseModel):
    ...
    rated_efficiency: Optional[Decimal] = Field(
        description="额定效率（%）", default=None, ge=0, le=100
    )
    ...
    min_frequency: Optional[Decimal] = Field(
        description="最小频率（Hz）", default=None, ge=0, le=100
    )
    max_frequency: Optional[Decimal] = Field(
        description="最大频率（Hz）", default=None, ge=0, le=100
    )
