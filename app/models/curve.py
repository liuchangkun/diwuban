...
from app.models.base import BaseModel, CurveType, DeviceId, StationId, TimestampMixin, ValidationMixin
...
class Curve(BaseModel, TimestampMixin, ValidationMixin):
    ...
    curve_id: Optional[int] = Field(
        description="曲线ID（自增主键）",
        default=None
    )
    ...
    equation: Dict[str, Any] = Field(
        description="拟合方程（JSON格式的系数和模型信息）"
    )
    ...
    created_by: str = Field(
        description="创建者（算法名或用户）",
        max_length=50
    )
    
    source_data_range: Optional[str] = Field(
        description="拟合所用数据的时间范围",
        default=None,
        max_length=100
    )
    ...
    status: str = Field(
        description="曲线状态",
        default=CurveStatus.ACTIVE
    )
    
    data_point_count: Optional[int] = Field(
        description="拟合使用的数据点数",
        default=None,
        ge=0
    )
...
class CurveCreate(BaseModel, ValidationMixin):
    ...
    fitting_method: Optional[str] = Field(
        description="拟合方法",
        default=FittingMethod.POLYNOMIAL
    )
...
class CurveUpdate(BaseModel, ValidationMixin):
    ...
    equation: Optional[Dict[str, Any]] = Field(
        description="拟合方程",
        default=None
    )
...
class CurveEvaluationRequest(BaseModel, ValidationMixin):
    ...
    metrics: List[str] = Field(
        description="评估指标列表",
        default=["r_squared", "rmse", "mae", "mape"]
    )
...
class CurveEvaluationResult(BaseModel):
    ...
    evaluation_metrics: Dict[str, Decimal] = Field(
        description="评估指标结果"
    )
    
    data_points_evaluated: int = Field(
        description="评估数据点数",
        ge=0
    )
    
    is_still_valid: bool = Field(
        description="曲线是否仍然有效"
    )
