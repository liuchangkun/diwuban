    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if settings and hasattr(settings, 'logging'):
                ...
                _log_parameters = _log_parameters and detailed_config.enable_parameter_logging
                _log_performance = _log_performance and detailed_config.enable_performance_logging
            ...
            if _log_entry:
                ...
                if _log_parameters and (args or kwargs):
                    ...
                    if kwargs:
                        params["kwargs"] = {k: sanitize_value(v) for k, v in kwargs.items()}
                ...
                if _log_exit:
                    ...
                    if _log_memory:
                        ...
                        if end_memory and start_memory:
                            memory_delta = {
                                "rss_delta_mb": round(end_memory.get("rss_mb", 0) - start_memory.get("rss_mb", 0), 2),
...
class InternalStepLogger:
    ...
    def __init__(self, function_name: str, logger: Optional[logging.Logger] = None, settings: Optional[Any] = None):
    ...
    def step(self, description: str, result: Any = None, **kwargs):
        ...
        self.logger.log(log_level, f"步骤 {self.step_count}: {description}", extra=extra)
    ...
    def checkpoint(self, name: str, data: Optional[Dict[str, Any]] = None):
        ...
        if data:
            extra["checkpoint_data"] = {k: sanitize_value(v, max_length=100) for k, v in data.items()}
    ...
    def branch(self, condition: str, taken: bool, result: Any = None):
        ...
        self.logger.debug(f"分支条件: {condition} -> {'执行' if taken else '跳过'}", extra=extra)
    ...
    def iteration_start(self, loop_name: str, total_items: int):
        ...
        return IterationLogger(self.function_name, loop_name, total_items, self.logger, self.config)
    
    def validation(self, check_name: str, is_valid: bool, details: Optional[str] = None):
        """记录数据验证"""
    ...
    def transformation(self, transform_name: str, input_info: str, output_info: str):
        ...
        self.logger.info(f"数据转换: {transform_name} ({input_info} -> {output_info})", extra=extra)


class IterationLogger:
    """循环迭代日志记录器"""
    
    def __init__(self, function_name: str, loop_name: str, total_items: int, logger: logging.Logger, config: dict):
        self.function_name = function_name
    ...
    def update(self, count: int = 1, item_info: Optional[str] = None):
        ...
        # 按频率记录迭代进度
        if (self.current_iteration % self.config['iteration_log_frequency'] == 0 or 
            self.current_iteration >= self.total_items):
            
            elapsed_time = current_time - self.start_time
            progress_percent = (self.current_iteration / self.total_items * 100) if self.total_items > 0 else 0
            ...
            if self.current_iteration > 0:
                ...
                extra["estimated_remaining_ms"] = round(estimated_remaining_time * 1000, 2)
                extra["items_per_second"] = round(items_per_second, 2)
            
            self.logger.info(f"循环进度: {self.loop_name} {self.current_iteration}/{self.total_items} ({progress_percent:.1f}%)", extra=extra)
    ...
    def complete(self, summary: Optional[str] = None):
        ...
        self.logger.info(f"循环完成: {self.loop_name}, 耗时 {total_time:.2f}秒", extra=extra)


def create_internal_step_logger(function_name: str, logger: Optional[logging.Logger] = None, settings: Optional[Any] = None) -> InternalStepLogger:
    """创建内部步骤日志记录器"""
    return InternalStepLogger(function_name, logger, settings)


def log_business_milestone(milestone_name: str, details: Dict[str, Any], logger: Optional[logging.Logger] = None):
    """
    记录业务里程碑
    ...
    logger.info(f"业务里程碑: {milestone_name}", extra={
        "event": "business.milestone",
        "milestone_name": milestone_name,
        "details": {k: sanitize_value(v, max_length=200) for k, v in details.items()}
    })


def log_data_quality_check(check_name: str, passed: bool, metrics: Dict[str, Any], logger: Optional[logging.Logger] = None):
    """
    记录数据质量检查结果
    ...
    logger.log(level, f"数据质量检查: {check_name} - {status}", extra={
        "event": "data.quality.check",
        "check_name": check_name,
        "passed": passed,
        "metrics": {k: sanitize_value(v, max_length=100) for k, v in metrics.items()}
    })
...
class ProgressLogger:
    ...
    def __init__(self, operation: str, total: int, logger: Optional[logging.Logger] = None):
        ...
        # 记录开始
        self.logger.info(f"开始{operation}", extra={
    ...
    def update(self, count: int = 1, message: str = ""):
        ...
        if (current_time - self.last_log_time >= self.log_interval or 
            ...
            extra = {
                ...
                "estimated_remaining_seconds": round(remaining_time, 2) if remaining_time > 0 else 0
            }
            
            if message:
                extra["message"] = message
            
            self.logger.info(f"{self.operation}进度: {self.current}/{self.total} ({progress_percent:.1f}%)", 
                           extra=extra)
    ...
    def complete(self, message: str = ""):
        ...
        extra = {
            ...
            "items_per_second": round(self.total / total_time, 2) if total_time > 0 else 0
        }
        
        if message:
            extra["message"] = message
        
        self.logger.info(f"{self.operation}完成: {self.total}项, 耗时{total_time:.2f}秒", extra=extra)


def create_progress_logger(operation: str, total: int, logger: Optional[logging.Logger] = None) -> ProgressLogger:
    """创建进度日志记录器"""
...
def log_key_metrics(
    ...
    logger.info(f"关键指标 - {operation}", extra={
        "event": f"key_metrics.{operation}",
        "metrics": metrics
    })
...
def log_file_processing_metrics(
    ...
    metrics = {
        ...
        "files_per_second": round(file_count / processing_time_seconds, 2) if processing_time_seconds > 0 else 0,
        "mb_per_second": round(total_size_mb / processing_time_seconds, 2) if processing_time_seconds > 0 else 0
    }
...
def log_data_processing_metrics(
    ...
    metrics = {
        ...
        "processing_time_seconds": processing_time_seconds,
        "rows_per_second": round(rows_processed / processing_time_seconds, 2) if processing_time_seconds > 0 else 0,
        "success_rate": round(rows_valid / rows_processed * 100, 2) if rows_processed > 0 else 0,
...
def log_merge_statistics(
    ...
    metrics = {
        ...
        "deduplication_rate": round(duplicates_removed / input_rows * 100, 2) if input_rows > 0 else 0,
        "rows_per_second": round(output_rows / merge_time_seconds, 2) if merge_time_seconds > 0 else 0,
        "data_time_range": data_time_range
    }
...
def log_sql_execution(
    ...
    if error:
        ...
        logger.error(f"SQL执行失败 - {sql_type}: {sql_summary}", extra={
            "event": "sql.execution.failed",
            **log_data
        })
    else:
        logger.info(f"SQL执行成功 - {sql_type}: {sql_summary}", extra={
            "event": "sql.execution.success",
            **log_data
        })
...
def log_sql_statement(
    ...
    if logger.isEnabledFor(logging.DEBUG):
        ...
        if parameters:
            ...
            safe_params = {}
            for key, value in parameters.items():
                if any(sensitive in str(key).lower() for sensitive in ['password', 'token', 'secret']):
                    safe_params[key] = "***"
                else:
                    safe_params[key] = sanitize_value(value, max_length=100)
            log_data["parameters"] = safe_params
        
        logger.debug("执行SQL语句", extra={
            "event": "sql.statement.debug",
            **log_data
        })
...
def create_sql_logger(operation: str) -> Callable:
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            logger.debug(f"开始{operation} - {func_name}", extra={
            ...
            try:
                ...
                logger.info(f"{operation}成功 - {func_name}", extra={
                ...
                logger.error(f"{operation}失败 - {func_name}: {e}", extra={
