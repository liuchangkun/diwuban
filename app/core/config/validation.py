...
class ConfigValidator:
    ...
    def validate_database_config(db_config: Dict[str, Any]) -> ValidationResult:
        ...
        if not name or not isinstance(name, str):
            result.add_error("db.name", name, "数据库名称不能为空且必须是字符串")
        elif not name.replace("_", "").replace("-", "").isalnum():
            result.add_warning("db.name", name, "数据库名称建议只包含字母、数字、下划线和连字符")
        ...
        if isinstance(pool_config, dict):
            ...
            if not isinstance(min_size, int) or min_size < 0:
                result.add_error("db.pool.min_size", min_size, "连接池最小大小必须是非负整数")
            
            if not isinstance(max_size, int) or max_size < 1:
                result.add_error("db.pool.max_size", max_size, "连接池最大大小必须是正整数")
            
            if isinstance(min_size, int) and isinstance(max_size, int) and min_size > max_size:
                result.add_error("db.pool", f"min_size={min_size}, max_size={max_size}", 
                               "连接池最小大小不能大于最大大小")

        # 验证超时配置
        timeouts = db_config.get("timeouts", {})
        if isinstance(timeouts, dict):
            timeout_fields = ["connect_timeout_ms", "statement_timeout_ms", "query_timeout_ms"]
            for field in timeout_fields:
                value = timeouts.get(field, 0)
                if not isinstance(value, int) or value < 0:
                    result.add_error(f"db.timeouts.{field}", value, f"{field}必须是非负整数（毫秒）")
    ...
    def validate_web_config(web_config: Dict[str, Any]) -> ValidationResult:
        ...
        if isinstance(server_config, dict):
            ...
            if not isinstance(host, str) or not host.strip():
                result.add_error("web.server.host", host, "Web服务器主机地址必须是非空字符串")

            # 验证端口
            port = server_config.get("port", 8000)
            if not isinstance(port, int) or port < 1 or port > 65535:
                result.add_error("web.server.port", port, "Web服务器端口必须是1-65535之间的整数")
            elif port < 1024:
                result.add_warning("web.server.port", port, "使用小于1024的端口可能需要管理员权限")

            # 验证工作进程数
            workers = server_config.get("workers", 1)
            if not isinstance(workers, int) or workers < 1:
                result.add_error("web.server.workers", workers, "工作进程数必须是正整数")
    ...
    def validate_system_config(system_config: Dict[str, Any]) -> ValidationResult:
        ...
        if isinstance(timezone_config, dict):
            valid_timezones = ["UTC", "Asia/Shanghai", "Asia/Beijing", "America/New_York", "Europe/London"]
            
            default_tz = timezone_config.get("default", "")
            if not isinstance(default_tz, str) or default_tz.strip() == "":
                result.add_error("system.timezone.default", default_tz, "默认时区不能为空")
            elif default_tz not in valid_timezones:
                result.add_warning("system.timezone.default", default_tz, 
        ...
        if isinstance(directories_config, dict):
            ...
            for dir_name in required_dirs:
                ...
                if not isinstance(dir_path, str) or dir_path.strip() == "":
                    result.add_error(f"system.directories.{dir_name}", dir_path, 
                                   f"{dir_name}目录路径不能为空")
    ...
    def validate_ingest_config(ingest_config: Dict[str, Any]) -> ValidationResult:
        ...
        if not isinstance(workers, int) or workers < 1 or workers > 100:
            result.add_error("ingest.workers", workers, "工作线程数必须是1-100之间的整数")
        elif workers > 20:
            result.add_warning("ingest.workers", workers, "工作线程数较大，可能影响性能")

        # 验证提交间隔
        commit_interval = ingest_config.get("commit_interval", 1000000)
        if not isinstance(commit_interval, int) or commit_interval < 1000:
            result.add_error("ingest.commit_interval", commit_interval, "提交间隔必须是大于等于1000的整数")
        ...
        if isinstance(csv_config, dict):
            ...
            if not isinstance(encoding, str) or encoding.lower() not in [e.lower() for e in valid_encodings]:
                result.add_error("ingest.csv.encoding", encoding, f"编码格式必须是: {valid_encodings}")

        # 验证批处理配置
        batch_config = ingest_config.get("batch", {})
        if isinstance(batch_config, dict):
            batch_size = batch_config.get("size", 50000)
            if not isinstance(batch_size, int) or batch_size < 100 or batch_size > 1000000:
                result.add_error("ingest.batch.size", batch_size, "批次大小必须是100-1000000之间的整数")
    ...
    def validate_merge_config(merge_config: Dict[str, Any]) -> ValidationResult:
        ...
        if isinstance(window_config, dict):
            ...
            if isinstance(window_size, str):
                ...
                if not any(window_size.endswith(unit) for unit in valid_units):
                    result.add_error("merge.window.size", window_size, f"时间窗口单位必须是: {valid_units}")
                else:
                    try:
                        value = int(window_size[:-1])
                        if value <= 0:
                            result.add_error("merge.window.size", window_size, "时间窗口值必须是正整数")
                    except ValueError:
                        result.add_error("merge.window.size", window_size, "时间窗口格式错误，应为数字+单位格式")
        ...
        if isinstance(segmented_config, dict):
            ...
            if granularity not in valid_granularities:
                result.add_warning("merge.segmented.granularity", granularity, 
    ...
    def validate_logging_config(logging_config: Dict[str, Any]) -> ValidationResult:
        ...
        if not isinstance(format_type, str) or format_type not in valid_formats:
            result.add_error("logging.format", format_type, f"日志格式必须是: {valid_formats}")
        ...
        if not isinstance(routing, str) or routing not in valid_routings:
            result.add_error("logging.routing", routing, f"日志路由必须是: {valid_routings}")
        ...
        if not isinstance(retention_days, int) or retention_days < 1 or retention_days > 365:
            result.add_error("logging.retention_days", retention_days, "日志保留天数必须是1-365之间的整数")
        elif retention_days < 7:
            result.add_warning("logging.retention_days", retention_days, "日志保留天数较短，建议至少7天")
        ...
        if isinstance(default_paths, dict):
            ...
            if mapping_file and isinstance(mapping_file, str):
                ...
                if not mapping_path.exists():
                    result.add_warning("ingest.default_paths.mapping_file", mapping_file, 
                                     f"映射文件不存在: {mapping_file}")
                elif not mapping_path.is_file():
                    result.add_error("ingest.default_paths.mapping_file", mapping_file, 
...
def log_validation_result(result: ValidationResult, logger: Optional[logging.Logger] = None) -> None:
    """
    记录验证结果日志
    ...
    for error in result.errors:
        logger.error(f"配置错误 [{error.field}]: {error.message}, 当前值: {error.value}")

    # 记录警告
    for warning in result.warnings:
        logger.warning(f"配置警告 [{warning.field}]: {warning.message}, 当前值: {warning.value}")

    if not result.is_valid:
        logger.error(f"配置验证失败: {len(result.errors)} 个错误, {len(result.warnings)} 个警告")
    else:
