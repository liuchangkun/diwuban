...
class HEtaCurve(BaseCurve):
    ...
    def extract_data(
        ...
        self._logger.info(f"提取H-η数据: device_id={device_id}")
        return pd.DataFrame(columns=["H", "eta"])

    def preprocess(self, data: pd.DataFrame) -> pd.DataFrame:
        if data.empty:
            return data
        clean = data.dropna(subset=["H", "eta"])
        clean = clean[(clean["H"] >= 0) & (clean["eta"] >= 0) & (clean["eta"] <= 100)]
        return clean
    ...
    def _do_fit(
        ...
        try:
            ...
            self._logger.error(f"H-η曲线拟合失败: {e}")
            return FitResult(
                success=False,
                curve_type=self.curve_type.value,
                method_id=method_id,
