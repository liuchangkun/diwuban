...
class CurveFittingService:
    ...
    async def fit_curve(self, request: CurveFittingRequest) -> CurveFittingResult:
        ...
        try:
            ...
            # 2. 数据清洗和过滤
            cleaned_data, outliers_count = self._preprocess_data(
                raw_data, request
            )
            ...
            with conn.cursor() as cur:
                ...
                for row in cur.fetchall():
                    ...
                    point = PumpOperationPoint(
                        ...
                        frequency=Decimal(str(frequency)) if frequency is not None else None
                    )
    ...
    def _preprocess_data(
        self, 
        raw_data: List[PumpOperationPoint], 
        ...
        if request.remove_outliers and np is not None:
            ...
            for i, point in enumerate(raw_data):
                # 检查是否为异常值
                is_outlier = (
                    flow_z_scores[i] > request.outlier_threshold or
                    power_z_scores[i] > request.outlier_threshold
                )
    ...
    def _is_valid_operating_point(
        ...
        if request.operating_range_only:
            # 仅保留正常运行工况（可根据实际需要调整）
            if (
                point.frequency is not None and 
    ...
    def _extract_fitting_variables(
        ...
        if np is not None:
            if curve_type == CurveType.N_Q:
                ...
                x_data = np.array([float(point.flow_rate) for point in data])
                y_data = np.array([
                    float(point.head) for point in data 
                    if point.head is not None
                ])
    ...
    def _generate_polynomial_formula(self, coefficients: List[float], intercept: float) -> str:
        """
        生成多项式公式字符串
        """
        terms = []
