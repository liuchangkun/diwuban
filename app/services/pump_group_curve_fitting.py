...
class PumpGroupCurveFitter:
    ...
    async def _fit_single_curve(
        self, curve_type: PumpGroupCurveType, data: pd.DataFrame
    ) -> Dict:
        """拟合单条曲线"""
    ...
    def _calc_efficiency_load(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for time_point, group in data.groupby("metric_time"):
            ...
            avg_efficiency = (
                (group["pump_efficiency"] * group["pump_flow_rate"]).sum() / total_flow
                if total_flow > 0
                else 0
            )
            ...
            group_metrics.append(
                {"load_ratio": load_ratio, "avg_efficiency": avg_efficiency}
            )
    ...
    def _calc_coordination(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for time_point, group in data.groupby("metric_time"):
            ...
            coordination_metrics.append(
                {"pump_count": pump_count, "coordination_coeff": coordination}
            )
    ...
    def _calc_imbalance(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for time_point, group in data.groupby("metric_time"):
            ...
            actual_eff = (
                group["pump_efficiency"] * group["pump_flow_rate"]
            ).sum() / flows.sum()
            efficiency_loss = ideal_eff - actual_eff

            imbalance_metrics.append(
                {"flow_ratio": flow_ratio, "efficiency_loss": efficiency_loss}
            )
    ...
    def _calc_frequency_distribution(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for time_point, group in data.groupby("metric_time"):
            ...
            freq_metrics.append(
                {"frequency_std": freq_std, "group_efficiency": group_eff}
            )

        return pd.DataFrame(freq_metrics)

    def _extract_xy_data(
        self, curve_type: PumpGroupCurveType, data: pd.DataFrame
    ...
    async def _fit_with_fallback(
        self, x_data: np.ndarray, y_data: np.ndarray, curve_type: PumpGroupCurveType
    ) -> Dict:
        """降级拟合策略"""
    ...
    async def _fit_method(
        self,
        method: str,
        x_data: np.ndarray,
        y_data: np.ndarray,
        curve_type: PumpGroupCurveType,
    ) -> Dict:
        """具体拟合方法"""
        ...
        if method == "polynomial":
            ...
            for degree in range(1, min(4, len(x_data) // 3)):
                try:
                    ...
                    score = 1 - np.sum((y_data - y_pred) ** 2) / np.sum(
            ...
            return {"type": "linear", "coefficients": coeffs}

        else:
            return self._get_default_curve(curve_type)

    def _evaluate_quality(
        self, model: Dict, x_data: np.ndarray, y_data: np.ndarray
    ) -> CurveQuality:
        """评估拟合质量"""
        ...
        # 计算指标
        r2 = 1 - np.sum((y_data - y_pred) ** 2) / np.sum(
            (y_data - np.mean(y_data)) ** 2
        )
        ...
        return CurveQuality(
            r2_score=r2, rmse=rmse, sample_count=len(x_data), confidence=confidence
        )
...
class CurveEnhancedCompletion:
    ...
    async def enhance_data_completion(
        self, station_id: str, missing_data: Dict, curve_models: Optional[Dict] = None
    ) -> Dict:
        """使用拟合曲线增强数据补齐"""
        ...
        for metric_type, missing_points in missing_data.items():
            ...
            if not relevant_curves:
                # 回退到传统方法
                enhanced_data[metric_type] = await self._traditional_completion(
                    missing_points
                )
                continue
            ...
            for point in missing_points:
                # 获取同时刻的已知指标
                known_metrics = await self._get_known_metrics(
    ...
    def _select_relevant_curves(self, metric_type: str, curve_models: Dict) -> Dict:
        ...
        return {
            name: curve_models[name] for name in relevant_names if name in curve_models
        }
    ...
    async def _predict_from_curves(
        self, target_metric: str, known_metrics: Dict, curve_models: Dict
    ) -> float:
        """基于曲线预测缺失值"""
        ...
        for curve_name, model in curve_models.items():
            try:
                ...
                if input_value is not None:
                    ...
                    weight = model.get(
                        "quality", CurveQuality(0.5, 5.0, 0, 0.5)
                    ).confidence
    ...
    def _calculate_confidence(
        self, prediction: float, known_metrics: Dict, curve_models: Dict
    ) -> float:
        """计算预测置信度"""
