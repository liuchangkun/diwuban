...
def backup_logs_directory(logs_dir: Path, backup_count: int) -> None:
    ...
    if not logs_dir.exists():
        logger.info("日志目录不存在，跳过备份", extra={
            "event": "cleanup.logs.backup.skip",
...
def cleanup_old_backups(parent_dir: Path, prefix: str, keep_count: int) -> None:
    ...
    backup_dirs = [d for d in parent_dir.iterdir() if d.is_dir() and d.name.startswith(prefix)]
    ...
    for backup_dir in dirs_to_remove:
        try:
            logger.info("删除旧备份目录", extra={
                "event": "cleanup.backup.remove",
                "backup_dir": str(backup_dir)
            })
            shutil.rmtree(backup_dir)
        except Exception as e:
            logger.warning("删除旧备份目录失败", extra={
...
def clear_logs_directory(logs_dir: Path, backup_count: int) -> None:
    ...
    try:
        if logs_dir.exists():
            ...
            logger.info("日志目录清理完成", extra={
                "event": "cleanup.logs.completed",
                "logs_dir": str(logs_dir),
                "files_removed": file_count
            })
        else:
            logger.info("日志目录不存在，跳过清理", extra={
                "event": "cleanup.logs.skip",
                "logs_dir": str(logs_dir)
            })
    
    except Exception as e:
        logger.error("清理日志目录失败", extra={
...
def clear_database(settings: Settings) -> None:
    ...
    Args:
        settings: 应用配置
    """
    logger.info("开始清理数据库", extra={
        "event": "cleanup.database.start",
    ...
    try:
        with get_conn(settings) as conn:
            with conn.cursor() as cur:
                ...
                if not tables:
                    logger.info("数据库中没有表，跳过清理", extra={
                        "event": "cleanup.database.skip",
                        "database": settings.db.name
                    })
                    return
                
                logger.info("开始清空数据库表", extra={
...
def perform_startup_cleanup(settings: Settings) -> None:
    ...
    if cleanup_config.confirm_clear:
        ...
        if response not in ('y', 'yes'):
            logger.info("用户取消清理操作", extra={
                "event": "cleanup.startup.cancelled"
            })
            return
...
def get_system_status(settings: Settings) -> dict:
    ...
    if logs_dir.exists():
        try:
            ...
            status["logs_directory"]["file_count"] = len([f for f in files if f.is_file()])
            total_size = sum(f.stat().st_size for f in files if f.is_file())
            status["logs_directory"]["size_mb"] = round(total_size / (1024 * 1024), 2)
        except Exception as e:
            logger.warning("获取日志目录信息失败", extra={
                "event": "cleanup.status.logs.failed",
                "error": str(e)
            })
    ...
    try:
        with get_conn(settings) as conn:
            with conn.cursor() as cur:
                ...
                # 获取总行数（仅统计主要表）
                main_tables = ['fact_measurements', 'staging_raw', 'dim_stations', 'dim_devices', 'dim_metric_config']
                total_rows = 0
        ...
        logger.warning("获取数据库信息失败", extra={
            "event": "cleanup.status.database.failed",
            "error": str(e)
        })
