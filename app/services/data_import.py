...
class DataImportService:
    ...
    async def import_csv_file(
        self, file_path: Path, station_id: str, source_hint: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        导入 CSV 文件
    ...
    async def _import_to_staging(
        self, valid_rows: List[ValidRow], rejected_rows: List[RejectRow]
    ) -> Dict[str, Any]:
        """
        导入数据到 staging 表
        """
    ...
    async def cleanup_old_staging_data(self, days_to_keep: int = 7) -> Dict[str, Any]:
        """
        清理老旧的 staging 数据
        """
        ...
        with get_conn(self.settings) as conn:
            with conn.cursor() as cur:
                # 清理 staging_raw
                cur.execute(
                    "DELETE FROM staging_raw WHERE loaded_at < %s", (cutoff_date,)
                )
                deleted_staging = cur.rowcount

                # 清理 staging_rejects
                cur.execute(
                    "DELETE FROM staging_rejects WHERE rejected_at < %s", (cutoff_date,)
                )
