...
def run_all(
    ...
    if cleanup_config.clear_database or cleanup_config.clear_logs:
        logger.info("执行run-all前置清理", extra={
        ...
        try:
            ...
            logger.info("run-all前置清理完成", extra={
                "event": "run_all.pre_cleanup.completed",
                "clear_database": cleanup_config.clear_database,
                "clear_logs": cleanup_config.clear_logs
            })
        except Exception as e:
            logger.error("run-all前置清理失败", extra={
                "event": "run_all.pre_cleanup.failed",
                "error": str(e)
            })
            # 清理失败不中止流程，仅记录错误
            pass
    else:
        logger.info("跳过run-all前置清理", extra={
    ...
    step_logger.step("初始化运行环境和日志系统", run_dir=str(run_dir) if run_dir else "auto")
    ...
    step_logger.checkpoint("env_setup_complete", {
        "run_id": run_id,
        "run_dir": str(run_dir),
    ...
    log_key_metrics("准备维表", {
        "duration_seconds": round(prepare_time, 2),
        "mapping_file": str(mapping)
    }, logger)
    ...
    log_key_metrics("创建临时表", {
        "duration_seconds": round(staging_time, 2)
    }, logger)
    ...
    # 记录文件处理指标
    if copy_stats:
        step_logger.validation("文件导入结果验证", 
    ...
    step_logger.step("数据合并完成", 
                    result=f"耗时 {merge_ms/1000:.2f} 秒",
                    merged_rows=merge_stats.get("rows_merged", 0) if isinstance(merge_stats, dict) else 0)
    ...
    if merge_stats and isinstance(merge_stats, dict):
        ...
        step_logger.validation("数据合并结果验证", 
                              output_rows > 0,
    ...
    try:
        with get_conn(settings) as conn:
            ...
            start_utc_str = window_start_utc.astimezone(timezone.utc).isoformat(timespec="seconds").replace("+00:00", "Z")
            end_utc_str = window_end_utc.astimezone(timezone.utc).isoformat(timespec="seconds").replace("+00:00", "Z")
    ...
    log_key_metrics("管道执行完成", {
        ...
        "total_files_processed": copy_stats.get("files_total", 0) if copy_stats else 0,
        "total_rows_processed": copy_stats.get("rows_read", 0) if copy_stats else 0,
        "final_rows_merged": merge_stats.get("rows_merged", 0) if isinstance(merge_stats, dict) else 0,
