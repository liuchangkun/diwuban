...
class OptimizationService:
        ...
        with get_conn(self.settings) as conn:
            with conn.cursor() as cur:
                ...
                for row in cur.fetchall():
                    device_id, name, device_type, pump_type, rated_power, \
                    rated_flow, rated_head, status = row
    ...
    async def _optimize_pump_configuration(
        ...
        for device in pump_devices:
            current_freq = current_state["device_states"].get(
                device.device_id, {}
            ).get("frequency", 45.0)
            ...
            optimized_config[device.device_id] = {
                ...
                "power_estimate": (
                    ...
                    if device.rated_power else 0
                ),
                "flow_estimate": (
                    float(device.rated_flow) * (optimized_freq / 50.0)
                    if device.rated_flow else 0
                )
            }
