...
class Calculator:
    ...
    def __init__(self, param_manager=None, trace_id: Optional[str] = None):
        ...
        # 导入2个计算方法
        from .methods import equiv_coef, static_pressure

        # 方法映射字典
        self.methods = {
            'equiv_coef': equiv_coef.calculate_equiv_coef,
            'static_pressure': static_pressure.calculate_static_pressure
        }
    ...
    def calculate(
        ...
        Args:
            data: 过滤后的数据
            method_id: 方法ID（equiv_coef 或 static_pressure）
        ...
        # 统计计算结果
        if 'pump_inlet_pressure' in df_result.columns:
            valid_count = df_result['pump_inlet_pressure'].notna().sum()
            mean_value = df_result['pump_inlet_pressure'].mean()
            max_value = df_result['pump_inlet_pressure'].max()
            min_value = df_result['pump_inlet_pressure'].min()
