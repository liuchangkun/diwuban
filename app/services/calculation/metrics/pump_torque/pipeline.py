...
class PumpTorquePipeline:
    ...
    def execute(
        ...
        max_head = params.get('max_head')

        # 验证必需参数存在性
        if max_power is None or max_speed is None:
            missing = []
            if max_power is None:
                missing.append('max_power')
            if max_speed is None:
                missing.append('max_speed')
        ...
        calc_result = calculator.calculate(filtered_data, selected_method)
        ...
        validated_result = validator.validate(calc_result, selected_method)
        valid_count = len(validated_result)
        invalid_count = len(calc_result) - valid_count
