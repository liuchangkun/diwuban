            ...
            if data_table:
                ...
                if rows:
                    ...
                    for key, value in first_row_data.items():
                        has_value = value and value != '-' and 'text-gray-400' not in value
                        status = '✅' if has_value else '❌'
                        print(f'  {status} {key}: {value}')
                    
                    # 统计有效数据字段
                    valid_fields = sum(1 for key, value in first_row_data.items() 
                                     if key not in ['timestamp', 'device_id'] 
                                     and value and value != '-' and 'text-gray-400' not in value)
                    ...
                    for i in range(min(5, len(rows))):
                        row_data = await page.evaluate(f'''() => {{
                        ...
                        if row_data:
                            print(f'  行 {i+1}: {row_data["validCount"]}/{row_data["total"]} 字段有值')
                else:
                    print('❌ 没有数据行')
            else:
                print('❌ 未找到数据表格')
