...
def export_csv():
    ...
    with psycopg.connect(dbname="pump_station_optimization", host="localhost", user="postgres") as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT id, name FROM public.dim_stations ORDER BY id")
            stations = {row[0]: row[1] for row in cur.fetchall()}
            cur.execute("SELECT id, station_id, name, type, pump_type FROM public.dim_devices ORDER BY station_id, id")
            devices = cur.fetchall()

    OUT.parent.mkdir(parents=True, exist_ok=True)
    with OUT.open("w", newline="", encoding="utf-8") as fp:
        w = csv.writer(fp)
        w.writerow(["device_id","station_id","station_name","device_name","pump_type","param_key","unit","example","zh_desc"])
        for dev_id, st_id, dev_name, dev_type, pump_type in devices:
            if dev_type == "main_pipeline":
                for key, unit, ex, zh in PARAM_ROWS_PIPELINE:
                    w.writerow([dev_id, st_id, stations.get(st_id, ""), dev_name, dev_type, key, unit or "", ex, zh])
            elif dev_type == "pump":
                # 根据变频/软启调整默认示例
                rows = list(PARAM_ROWS_COMMON)
                if pump_type == "variable_frequency":
                    rows[2] = ("rated_efficiency", None, 0.76, "额定效率(0-1)")  # 调低一点用于示例
                ...
                for key, unit, ex, zh in rows:
                    w.writerow([dev_id, st_id, stations.get(st_id, ""), dev_name, pump_type or dev_type, key, unit or "", ex, zh])
    print(f"已导出: {OUT}")
