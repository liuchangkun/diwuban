    ...
    try:
        ...
        with psycopg.connect(dsn) as conn:
            with conn.cursor() as cur:
                ...
                cur.execute("SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'fact_measurements' ORDER BY ordinal_position")
                for row in cur.fetchall():
                    print(f'  {row[0]}: {row[1]}')
                    
                # 检查dim_devices表结构
                print('\n=== dim_devices表结构 ===')
                cur.execute("SELECT column_name, data_type FROM information_schema.columns WHERE table_name = 'dim_devices' ORDER BY ordinal_position")
                for row in cur.fetchall():
                    print(f'  {row[0]}: {row[1]}')
                    
                # 检查dim_metric_config表中有哪些指标
                print('\n=== 前20个指标配置 ===')
                cur.execute("SELECT id, metric_key, unit, unit_display FROM dim_metric_config ORDER BY id LIMIT 20")
                for row in cur.fetchall():
                ...
                """)
                for row in cur.fetchall():
                    print(f'  station_id={row[0]}, device_id={row[1]}, metric_id={row[2]}, ts={row[3]}, value={row[4]}, metric={row[5]}, unit={row[6]}')
