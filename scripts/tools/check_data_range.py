    ...
    try:
        ...
        with get_conn(settings) as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT MIN(timestamp), MAX(timestamp) FROM v_fully_adaptive_data"
                )
                ...
                # 检查特定泵站的数据
                cur.execute(
                    "SELECT DISTINCT station_id, station_name FROM v_fully_adaptive_data"
                )
                ...
                if stations:
                    ...
                    cur.execute(
                        "SELECT MIN(timestamp), MAX(timestamp) FROM v_fully_adaptive_data WHERE station_id = %s",
                        (first_station_id,),
                    )
