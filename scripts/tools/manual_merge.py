...
def manual_merge():
    try:
        ...
        print(
            f"解析结果: {parsed_result[0]} 行, 时间范围: {parsed_result[1]} 到 {parsed_result[2]}"
        )
        ...
        cur.execute(
            dedup_sql, ("Asia/Shanghai", start_utc, end_utc, start_utc, end_utc)
        )
        dedup_result = cur.fetchone()
        print(
            f"去重结果: 总行数={dedup_result[0]}, 唯一行数={dedup_result[1]}, 窗口内={dedup_result[2]}, 窗口内唯一={dedup_result[3]}"
        )
