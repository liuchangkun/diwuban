...
CREATE_PAT = re.compile(r'(?is)\bCREATE\s+(TABLE|MATERIALIZED\s+VIEW|VIEW|INDEX)\b[\s\S]*?;')
...
def main():
    ...
    for p in sorted(SQL_DIR.glob('*.sql')):
        ...
        if not blocks:
            continue
        print(f'## {p.name}\n')
        for m in re.finditer(r'(?is)\bCREATE\s+(TABLE|MATERIALIZED\s+VIEW|VIEW|INDEX)\b[\s\S]*?;', text):
