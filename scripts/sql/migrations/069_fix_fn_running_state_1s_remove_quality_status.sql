...
/*
迁移脚本: 069_fix_fn_running_state_1s_remove_quality_status.sql
创建时间: 2025-11-11
迁移目的: 修复 fn_running_state_1s 函数中引用不存在的 quality_status 列的问题

问题描述:
  - 数据库中的 fn_running_state_1s 函数包含 "AND f.quality_status = 0" 条件
  - fact_measurements 表没有 quality_status 列
  - 导致函数执行失败，mv_device_running_1s 表无法填充

修复方案:
  - 使用 scripts/sql/m1/010_fn_running_state_1s.sql 的正确版本（SQL语言，无quality_status引用）
  - 替换数据库中的 plpgsql 版本

影响范围:
  - fn_running_state_1s 函数
  - sp_refresh_mv_running_phase 存储过程（调用此函数）
  - DeviceRunningJob（依赖此函数）
*/

-- 删除旧版本（如果存在）
DROP FUNCTION IF EXISTS public.fn_running_state_1s(bigint, bigint, timestamptz, timestamptz);

-- 创建新版本（基于 scripts/sql/m1/010_fn_running_state_1s.sql）
