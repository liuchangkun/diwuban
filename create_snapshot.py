...
def create_snapshot():
    ...
    try:
        ...
        # 获取表行数
        tables = ['dim_stations', 'dim_devices', 'dim_metric_config', 'fact_measurements', 'staging_raw']
        for table in tables:
        ...
        # 获取数据时间范围
        cur.execute('SELECT MIN(ts_bucket), MAX(ts_bucket) FROM fact_measurements WHERE ts_bucket IS NOT NULL')
