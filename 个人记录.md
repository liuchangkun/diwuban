...
数据库连接池失效 - 所有操作都在使用直连模式，严重影响性能
数据合并性能瓶颈 - 单次合并耗时16-17秒，占总执行时间75%
前置清理功能缺陷 - 无法清理运行中的日志文件，清理机制不完善
数据重复率异常 - 50%的数据被去重，需要分析原因
性能统计错误 - duration_ms计算错误，导致性能分析失真
监控数据缺失 - 关键性能指标(背压、批次统计)未正确收集
